import Shortend_url_model from "@shared/models/shortend_url.model";
...
export const create_shortned_url = async (req: Request, res: Response) => {
...
export const editShortnerUrl = async (req: Request, res: Response) => {
  const userId = req?.user?.userId;
...
export const deleteShortendUrl = async (req: Request, res: Response) => {
  const userId = req?.user?.userId;
  const shortendUrlId = req.params?.id;
  const temp = await Shortend_url_model.findById(shortendUrlId);
